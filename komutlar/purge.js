...
sayı++
  ...
  if(sayı <= 0) return message.channel.send("Mesaj bulunamadı, 2haftadan önce atıldıklarına emin ol");
...
message.channel.send(`${await data.fetch(`sizee.${message.id}`)} tane mesaj sildim\n\n${nbr.join('\n')}`)
